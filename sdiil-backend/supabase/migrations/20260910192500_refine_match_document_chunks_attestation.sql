-- Migration: Refine match_document_chunks attestation filtering
-- Enforces:
-- 1. REJECTED documents never match for any user
-- 2. PENDING_REVIEW documents match ONLY for uploader or SUPERVISOR/ADMIN
-- 3. ACTIVE documents match for all assigned users within sensitivity clearance
...
BEGIN
  ...
  WHERE
    ...
    AND de.sensitivity_level = ANY(allowed_sensitivities)
